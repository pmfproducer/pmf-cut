"""PMF Cut preview server — serves the standard editing interface + session media.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        _t = body.get("type")
        name = (
            "preview_style.json" if _t == "style-setup"
            else "preview_music.json" if _t == "music"
            else "preview_approve.json" if _t == "approve"
...
def main() -> None:
    ap = argparse.ArgumentParser(description="PMF Cut preview interface server")
    ...
    print(f"PMF Cut preview → http://127.0.0.1:{args.port}  (root: {root})", flush=True)
    srv.serve_forever()
