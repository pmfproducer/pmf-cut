...
def music_digest(p: Path) -> str:
    """BGM volume adjusted from the soundtrack track slider."""
    try:
        d = json.loads(p.read_text())
    except (OSError, json.JSONDecodeError) as e:
        return f"preview_music.json ilegível ({e.__class__.__name__}) — peça ao usuário para salvar de novo"
    vol = d.get("musicVolume")
    pct = f"{round(float(vol) * 100)}%" if vol is not None else "?"
    return (
        f"VOLUME DA TRILHA AJUSTADO NO PREVIEW ({d.get('savedAt', '')}) — "
        f"regrave o final.mp4 com a música em {pct} (volume={vol})."
    )


def approve_digest(p: Path) -> str:
    """User clicked 'Aprovar Fase 1' in the preview."""
    try:
        d = json.loads(p.read_text())
    except (OSError, json.JSONDecodeError):
        d = {}
    return (
        f"FASE 1 APROVADA NO PREVIEW ({d.get('savedAt', '')}) — "
        f"abra a aba Estilo (state.awaitingStyle=true) para a Fase 2."
    )
...
def main() -> int:
    ...
    watched = {
        ...
        root / "preview_music.json": music_digest,
        root / "preview_approve.json": approve_digest,
    }
