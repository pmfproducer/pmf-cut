...
`adelay=33` is one frame at 30fps, matching the `VIDEO_LAG` the overlays use — but
**that number is a per-project measurement, not a constant** (see "videoLag" in
`assets/shortform/README.md`). Measure it, then use the SAME value here and in the
SFX placement. When the measurement says 0, drop the delay entirely: on a 24fps
project that inherited `adelay=1 frame` against a true lag of 0, the delivered voice
sat 41.7ms late and the user heard it as soft lip-sync drift.
`-t "$VD"` keeps the audio from outliving the video.

**Verify** by correlating the delivered voice against `cut.mp4` at three points
with 15s windows (short windows lock onto the wrong syllable and invent a drift).
**A CONSTANT offset is not a PASS.** Constant proves there is no drift; it says
nothing about whether the offset should be there. Read it against the measured
lag: constant +41.7ms is correct only if the lag is genuinely 1 frame, and is a
one-frame desync otherwise. A rising correlation peak when you remove a wrong
delay (0.987 → 0.995 measured) is the confirmation that it was wrong.

Drop this re-mux ONLY if Phase 2 baked SFX into the audio (stacked captions'
click/scratch), and then verify sync by hand.

### `adelay=NS` counts samples at the INPUT's rate — resample first

The single nastiest bug in this pipeline. `adelay=803440S` does **not** mean
803440/48000 s unless that input is already 48kHz. The shipped sfx pack is mixed:
`cut-click.mp3` is 48kHz, `whoosh.mp3` is 44.1kHz. Delaying both with sample counts
computed at 48kHz put every whoosh 8.84% late — **+1.5s by the 17s mark**, heard as
the transition sound arriving well after the picture changed. The clicks were
correct, which is exactly what made it hard to see: half the transitions were fine.

Always `aresample=48000` **before** `adelay`:

```
[3:a]aresample=48000,volume=0.5,adelay=<samples>S:all=1[s3]
```

**Verify SFX placement on an SFX-ONLY bus**, never against the finished mix —
subtracting two differently-loudnorm'd signals with music on top measures the
level difference, not the effect, and it reported nonsense (+340ms, −140ms,
−240ms on transitions that were actually fine). Render the same graph into
silence (`anullsrc`) and find each peak:

```bash
ffmpeg -y -f lavfi -t <dur> -i anullsrc=r=48000:cl=mono <sfx inputs> \
  -filter_complex "<same aresample+volume+adelay chain>;...amix=...[mix]" \
  -map "[mix]" -ar 48000 /tmp/sfxbus.wav
```

Every transition should peak within ~10ms of its cut. Also **anchor a slow-attack
SFX by its PEAK, not its file start**: `whoosh.mp3` peaks 220ms in, so starting it
on the sweep lands the audible hit after the cut. Measure the envelope
(10ms RMS windows) and offset by it. A click peaks at 0ms and needs none of this.
