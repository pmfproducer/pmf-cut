/* PMF Cut preview — interactive editing timeline.
 ...
 * The one place that knows which looks PMF Cut can build. It is APP-level, not
...
const STYLE_CATALOG = {
  ...
  elements: [
    ...
    {
      id: 'swipeCut',
      name: 'Swipe na transição',
      def: false,
      icon: '<svg viewBox="0 0 16 16"><path d="M2 8h9" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" fill="none"/><path d="M8 4.2L11.8 8 8 11.8" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round" fill="none"/><path d="M2 4.6h3M2 11.4h3" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" fill="none" opacity=".5"/></svg>',
    },
    {
      id: 'musicAI',
...
let S = {
  ...
  jcutOpen: localStorage.getItem('pmfcut.jcutOpen') === '1',
};
...
async function applyState(data) {
  ...
  $('projectName').textContent = S.state.project || 'PMF Cut';
...
function renderChips() {
  ...
  // soundtrack → its own track with a LIVE volume slider for the BGM. The slider
  // saves to preview_music.json (its own file, so it never clobbers notes/edl),
  // and the skill re-muxes final.mp4 at the new volume — no re-render needed.
  const mus = (S.state && S.state.music) || (S.editData && S.editData.soundtrack);
  if (mus && (mus.file || mus.present || mus.enabled)) {
    const trk = el('div', 'track', insertTracksEl);
    ...
    chip.style.left = '0px';
    chip.style.width = `${Math.max(dur * S.pps, 160)}px`;
    const name = (mus.file || 'trilha.mp3').split('/').pop();
    const vol = mus.volume != null ? +mus.volume : 0.12;
    el('span', 'music-name', chip).textContent = name;
    const sl = el('input', 'music-vol', chip);
    sl.type = 'range'; sl.min = '0'; sl.max = '100'; sl.step = '1';
    sl.value = String(Math.round(vol * 100));
    sl.title = 'Volume da trilha (BGM)';
    const pct = el('span', 'music-pct', chip);
    pct.textContent = `${sl.value}%`;
    // keep the slider from starting a timeline scrub
    sl.addEventListener('pointerdown', (e) => e.stopPropagation());
    sl.addEventListener('input', (e) => { e.stopPropagation(); pct.textContent = `${sl.value}%`; });
    sl.addEventListener('change', () => saveMusicVolume(+sl.value / 100));
  }
}

async function saveMusicVolume(v) {
  try {
    const res = await fetch('/api/save', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ type: 'music', musicVolume: v }),
    });
    if ((await res.json()).ok) {
      if (S.state.music) S.state.music.volume = v;
      toast(`✓ Volume da trilha: ${Math.round(v * 100)}% — o Claude vai regravar o vídeo`, 4500);
    } else {
      toast('Erro ao salvar o volume — o servidor está de pé?', 4000);
    }
  } catch (e) {
    toast('Erro ao salvar o volume', 4000);
  }
}
...
// ---------- Fase 1 approval button ----------
// Shows on Fase 1 once the cut exists and the style gate has not opened yet.
// Clicking it is the same as telling the Claude "aprovado" — it signals the skill
// (via preview_approve.json) to open the Estilo tab for Fase 2.
function refreshApprove() {
  const btn = $('btnApprove');
  if (!btn) return;
  const hasVideo = S.videoDuration > 0;
  const can = hasVideo && S.tab === 1 && !(S.state && S.state.awaitingStyle)
    && !(S.state && S.state.style) && ((S.state && S.state.phase) || 1) < 2;
...
$('btnApprove').addEventListener('click', async () => {
  try {
    ...
    if ((await res.json()).ok) {
      ...
      toast('✓ Fase 1 aprovada — o Claude vai abrir o Estilo da Fase 2', 5000);
    } else {
      toast('Erro ao aprovar — o servidor está de pé?', 4000);
    }
  } catch (e) {
    toast('Erro ao aprovar', 4000);
  }
});
...
$('jcutToggle').addEventListener('click', () => {
  ...
  localStorage.setItem('pmfcut.jcutOpen', S.jcutOpen ? '1' : '0');
  renderJcutAudio();
  updateScrollRange();
  positionNeedle();
});
