...
const ORANGE = (editData as {captions?: {accent?: string}}).captions?.accent || '#ff5200';
...
type CapWin = {start: number; end: number; stackedOffsetY: number};
type CapCfg = {stackedOffsetY?: number; fontScale?: number; sfx?: SfxCfg; windows?: CapWin[]};
...
const OFFSET_WINDOWS = CAP.windows ?? []; // per-segment overrides (split → above seam)
...
const Cue: React.FC<{cue: CueData; cueDurationFrames: number}> = ({cue, cueDurationFrames}) => {
  ...
  const cueStartSec = cue.startMs / 1000;
  const win = OFFSET_WINDOWS.find((w) => cueStartSec >= w.start && cueStartSec < w.end);
  const baseY = Math.round(height * (win ? win.stackedOffsetY : OFFSET_Y));
